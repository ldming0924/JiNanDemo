...
/**
 * Created by deming.liu on 2016/9/7.
 */
public class DrawChart3 extends View {
    ...
    int widthSize;
    int heightSize;
    private String yz = "1.6";  // y轴的最大值



    private List<MyPoint> plist;
    ...
    public DrawChart3(Context context) {
        this(context,null);
    }

    public DrawChart3(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }

    public DrawChart3(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void drawTable(Canvas canvas){
        ...
        //画左侧数字
        canvas.drawText(50+"", OFFSET_LEFT - TEXT_OFFSET -5, OFFSET_TOP+10, paint);
        // for(int i = 1 ; i<10 ; i++){
        //float f = (float) 0.3*i;
        // Log.d("FLOAT",f+"   ");
        // canvas.drawText(0.3+"", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/10*(i), paint);

        canvas.drawText("-20", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(1), paint);
        canvas.drawText("-10", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(2), paint);
        canvas.drawText("0", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(3), paint);
        canvas.drawText("10", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(4), paint);
        canvas.drawText("20", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(5), paint);
        canvas.drawText("30", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(6), paint);
        canvas.drawText("40", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(7), paint);

        // }
        canvas.drawText("-30", OFFSET_LEFT - TEXT_OFFSET -5,  CHARTH-100, paint);
        ...
        for(int i = 1 ; i<8 ; i++){
            path.moveTo(OFFSET_LEFT,  CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(i));  //moveTo 不会进行绘制，只用于移动移动画笔。把画笔移动(x,y)处开始绘制
            path.lineTo(CHARTW-100, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(i));//lineTo 用于进行直线绘制。添加一条从一点到指定点(x,y)的线，如果没有移至，则自动设置为(0,0)。
        ...
        try {
            ...
            if(plist.size() >= 2){
                for(int i = 0; i<plist.size()-1; i++){
                    canvas.drawLine((float) plist.get(i).getX(),(float) plist.get(i).getY(), (float)plist.get(i+1).getX(), (float)plist.get(i+1).getY(), paint);
                    invalidate();
                }
    ...
    private void prepareLine(double y){
        ...
        double f = ((double) y)/80; //计算y占y轴最大值得比值，表示将表格分成80份，每份有多少
        double f1 = 1- f;
        ...
        MyPoint p = new MyPoint();//将坐标保存在Point类,从最右边的点开始
        p.setX(CHARTW-100);
        p.setY(py);
