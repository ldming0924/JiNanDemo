...
public class DrawChart1 extends View{
    ...
    public DrawChart1(Context context) {
        this(context,null);
    }

    public DrawChart1(Context context, AttributeSet attrs) {
        this(context, attrs,0);
    }

    public DrawChart1(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void drawTable(Canvas canvas){
        ...
        //画左侧数字
        canvas.drawText(1.6+"", OFFSET_LEFT - TEXT_OFFSET -5, OFFSET_TOP+10, paint);
        for(int i = 1 ; i<8 ; i++){

            canvas.drawText((float)0.2*i+"", OFFSET_LEFT - TEXT_OFFSET-5, CHARTH-100-( CHARTH-OFFSET_TOP-100)/8*(i), paint);
        }
    ...
    private void prepareLine(double y){
        ...
        double f = ((double) y)/1.6; //计算y占y轴最大值得比值，表示将表格分成200份，每份有多少
        double f1 = 1- f;
